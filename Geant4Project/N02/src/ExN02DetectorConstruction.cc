...
//--------- Definitions of Solids, Logical Volumes, Physical Volumes ---------
  ...
  // == ZSection ==
  zsections.push_back(G4ExtrudedSolid::ZSection(-1, G4TwoVector(), 1.));
  zsections.push_back(G4ExtrudedSolid::ZSection(3, G4TwoVector(), 1.));
  G4ExtrudedSolid* extruded_solid = new G4ExtrudedSolid("sExtruded",
                                                        polygon,
                                                        zsections); 
  logicTarget = new G4LogicalVolume(extruded_solid,TargetMater,"Target",0,0,0);
  ...
  std::string m_filename = "g4extruded.gdml";
  G4LogicalVolume* lworld = logicWorld;
  bool m_store_refs = true;
  fParser->Write(m_filename, lworld, m_store_refs);
  //
  return physiWorld;
  // Quick TEST
  //
  G4double m_steelTubZbottom = -20*cm;
  G4double m_steelTubZtop = 10*cm;
  G4double m_steelTubR = 10*cm;

  static G4double  zPlane[] = {m_steelTubZbottom, m_steelTubZtop};
  static G4double  rInner[] = {0., 0.};
  static G4double  rOuter[] = {m_steelTubR, m_steelTubR};
  G4Polycone* solidBottomRock = new G4Polycone("sSteelTub", //const G4String& pName,
                                   0, //G4double  phiStart,
  ...
  logicTarget = new G4LogicalVolume(solidBottomRock,TargetMater,"Target",0,0,0);
  physiTarget = new G4PVPlacement(0,               // no rotation
				  G4ThreeVector(0,0,0),  // at (x,y,z)
				  logicTarget,     // its logical volume				  
				  "Target",        // its name
